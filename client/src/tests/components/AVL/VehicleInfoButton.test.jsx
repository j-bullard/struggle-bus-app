...
describe("<VehicleInfoButton />", () => {
  it("should open the drawer when the button is clicked", async () => {
    /*     const { baseElement: container } = render(
      <MemoryRouter initialEntries={["/avl"]}>
        <VehicleInfoButton trimId={mockVehicle.id} />,
      </MemoryRouter>,
    );
    ...
    const infoButton = screen.getByTestId("vehicle-info-button");
    await act(() => fireEvent.click(infoButton));

    expect(
      container.querySelector(".chakra-modal__content"),
    ).toBeInTheDocument(); */
  });
});
